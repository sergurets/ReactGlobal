...
import { sortBy, detailedSearch } from '../utilities/urls.ts';
...
class Search extends React.Component {
  ...
  onClick() {
    const data = this.state.value ? this.state.value.trim() : '';
    const url = data ? detailedSearch(data, this.state) : sortBy(this.state.sortBy);
  ...
  render() {
    const searchByTitle = this.state.search === 'title';
    const sortByDate = this.state.sortBy === 'release_date';
    return (
